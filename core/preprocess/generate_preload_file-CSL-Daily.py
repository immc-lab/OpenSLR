...
def load_info(prefix):
    train_info = load("./CSL-Daily/train_info.npy").item()
    test_info = load("./CSL-Daily/test_info.npy").item()
    dev_info = load("./CSL-Daily/dev_info.npy").item()
...
def generate(gloss_dict, info_list):
    for key in tqdm(info_list):
        if key != "prefix":
            ...
            target_path = source_path.replace("CSL-Daily_256x256px", "CSL-Daily_256x256px-pickle")
            print(source_path)
            print(target_path)
            ...
            image_data = torch.cat([torch.tensor(cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)) for img_path in image_list])
...
def main():
    gloss_dict = np.load("./CSL-Daily/gloss_dict.npy", allow_pickle=True).item()
    train_info_list, test_info_list, dev_info_list = load_info("/sda/home/guozihang/guozihang/CSL-Daily_256x256px")
